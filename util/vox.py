...
class Vox_util(object):
    def __init__(self, Z, Y, X, scene_centroid, bounds, pad=None, assert_cube=True):
    ...
    def Mem2Ref(self, xyz_mem, Z, Y, X, assert_cube=True):
    ...
    def get_ref_T_mem(self, B, Z, Y, X, assert_cube=True, device='cuda'):
        mem_T_ref = self.get_mem_T_ref(B, Z, Y, X, assert_cube=assert_cube, device=device)
    ...
    def get_mem_T_ref(self, B, Z, Y, X, assert_cube=True, device='cuda'):
    ...
    def get_inbounds(self, xyz, Z, Y, X, already_mem=False, padding=0.0, assert_cube=True):
        # xyz is B x N x 3
    ...
    def get_3d_flow(self, xyz_ori_ref, xyz_tar_ref, Z, Y, X, already_mem=False, assert_cube=True):
    ...
    def get_occupancy(self, xyz, Z, Y, X):
        # xyz is B x N x 3 and in mem coords
        # we want to fill a voxel tensor with 1's at these inds
        B, N, C = list(xyz.shape)
        assert(C==3)
    ...
    def apply_pixX_T_memR_to_voxR(self, pix_T_camX, camX_T_camR, voxR, D, H, W, noise_amount=0.0, grid_z_vec=None, logspace_slices=False, lrt=None):
        # mats are B x 4 x 4
