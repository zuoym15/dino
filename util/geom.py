...
def depth2pointcloud(z, pix_T_cam):
    ...
    device = z.device
    y, x = util.basic.meshgrid2d(B, H, W, device=device)
...
def convert_box_to_ref_T_obj(box3d):
    ...
    obj_T_ref = util.basic.matmul2(obj_T_center, center_T_ref)
...
def split_rtlist(rtlist):
    ...
    __p = lambda x: util.basic.pack_seqdim(x, B)
    __u = lambda x: util.basic.unpack_seqdim(x, B)
...
def apply_4x4_to_lrtlist(Y_T_X, lrtlist_X):
    ...
    rtlist_Y_ = util.basic.matmul2(Y_T_Xs_, rtlist_X_)
...
def apply_4x4s_to_lrts(Ys_T_Xs, lrt_Xs):
    ...
    rtlist_Y_ = util.basic.matmul2(Ys_T_Xs_, rtlist_X_)
...
def create_depth_image_single(xy, z, H, W, force_positive=True, max_val=100.0, serial=False, slices=20):
    ...
    inds = util.basic.sub2ind(H, W, xy[:,1], xy[:,0]).long()
    if not serial:
        depth[inds] = z
    else:
        if False:
        ...
        for si in slice_inds:
            mini_z = z[si]
            mini_xy = xy[si]
            inds = util.basic.sub2ind(H, W, mini_xy[:,1], mini_xy[:,0]).long()
            depth[inds] = mini_z
...
def get_iou_from_corresponded_lrtlists(lrtlist_a, lrtlist_b):
    ...
    for b in list(range(B)):
        for n in list(range(N)):
            iou_3d =  util.box.new_box3d_iou(lrtlist_a[b:b+1,n:n+1],lrtlist_b[b:b+1,n:n+1])
            _, iou_2d = util.box.box3d_iou(xyzlist_a[b,n], xyzlist_b[b,n]+1e-4)
...
def pointcloud2flow(xyz1, pix_T_cam, H, W):
    ...
    grid_y0, grid_x0 = util.basic.meshgrid2d(B, H, W)
...
def sincos_norm(sin, cos):
    ...
    both = util.basic.l2_normalize(both, dim=-1)
...
def get_image_inbounds(pix_T_cam, xyz_cam, H, W, padding=0.0):
    ...
    xy_pix = util.geom.apply_pix_T_cam(pix_T_cam, xyz_cam)
...
def get_NY_transforms_from_clist(clist, retain_pitch=False, use_endpoint=False, return_in_case=False, use_endpoint_in_case=False):
    ...
    if use_torch:
        ...
        # rot = util.geom.eul2rotm(yaw*0.0, yaw, pitch)
        rot = util.geom.eul2rotm(torch.zeros_like(yaw), yaw, pitch)
    else:
        delta = delta.detach().cpu().numpy()
        ...
        rot = [util.py.eul2rotm(0,y,p) for (p,y) in zip(pitch,yaw)]
        rot = np.stack(rot)
    ...
    t = -xyz0
    # t is B x 3
    zero_T_camX0 = util.geom.merge_rt(rot0, t)
    camNY_T_zero = util.geom.merge_rt(rot, t0)
    camNY_T_camX0 = util.basic.matmul2(camNY_T_zero, zero_T_camX0)
...
def get_point_correspondence_from_flow(xyz0, xyz1, flow_f, pix_T_cam, H, W, flow_valid=None):
    ...
    x0 = xy[:, :, 0] # B x N
    y0 = xy[:, :, 1] # B x N
    uv = util.samp.bilinear_sample2d(flow_f, x0, y0) # B x 2 x N
...
def fill_ray_single(xyz, target_T_given=None, ray_add=0.0, num_points=100):
    ...
    if target_T_given is not None:
        ...
        xyz = util.geom.apply_4x4(target_T_given, xyz)
...
def get_plane_mask(H, W, pix_T_cam, xyz_cam, max_dist=1):
    ...
    for b in range(B):
        ...
        xy_inl = util.geom.camera2pixels(xyz_cam_inl.unsqueeze(0), pix_T_cam[b:b+1]).squeeze(0) # num_inl x 2
